...
import matplotlib.backends.backend_pdf
    #run as python3 nameofthisfile.py seq01.fastqish seq09.fastqish seq21.fastqish seq33.fastqish

pdf = matplotlib.backends.backend_pdf.PdfPages("linear_structure_maps.pdf")

for x in sys.argv[1:]:

    lst_cleaned = []
    #for arg in sys.argv[1:]:
    areg = pd.read_table(x, header = None)
    ...
    class SheetPlotter(graphics.FeaturePlotter):
        ...
        def draw(self, axes, feature, bbox, loc, style_param):
            ...
            axes.add_patch(biotite.AdaptiveFancyArrow(
                ...
                color=biotite.colors["orange"], linewidth=0 #if we want more color options, use "color= 'darkmagenta'" (calling on matplotlib instead of biotite)
            ))
    ...
    def visualize_secondary_structure(sse, first_id):
        ...
        fig = plt.figure(figsize=(60.0, 1.3))   #potentially make second char could be scalable..
        ...
        plt.title(just_seq_names) 
        fig.tight_layout()

    pdf.savefig(visualize_secondary_structure(sse, 2))
pdf.close()
