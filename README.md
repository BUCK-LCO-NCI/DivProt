...
The network cut-off scheme was built on base SS matrix output (Final_3_align...csv), however, you can absolutely use it on a weighted matrix of yours. As mentioned before, just be aware you'll likely be needing to play around with cutoff percentages with the -network_custom parameter. Additionally it should be noted that you can input a negative value as -tm (-0.5 = -50% scaling = 50% decrease to cutoff value = more edges between nodes). You may need to use this with a weighted matrix, though is likely still unncessary, as our base edge-weight cut-off is extremely conservative, and the 10-50% scaling retains this sentiment.
